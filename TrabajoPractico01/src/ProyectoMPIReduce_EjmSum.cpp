...
int main(int argc, char *argv[])
{
    ...
    double suma, sumafinal; // Valor local de los elementos del arreglo
   ...
   for(int i=0; i< (tama/size);i++ ){
	...
	MPI_Reduce(&suma, // Valor local de PI
			   &sumafinal,  // Dato sobre el que vamos a aplicar la operacion
			   1,	 // Numero de datos que vamos a reducir
			   MPI_DOUBLE,  // Tipo de dato que vamos a operar
			   MPI_SUM,  // Operacion que aplicaremos
			   0, // proceso que va a recibir las sumas
			   MPI_COMM_WORLD);
	...
	if (rank == 0){
		...
		cout << "El valor de la suma  de los datos aleatorios es: " <<sumafinal<<endl;
	}
