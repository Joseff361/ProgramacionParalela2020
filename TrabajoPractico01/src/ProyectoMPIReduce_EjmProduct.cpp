...
int main(int argc, char *argv[])
{
    ...
    double producto=1, productofinal; // Valor local de los elementos del arreglo
    ...
    if(rank==0){
    	  srand(time(NULL)); //semilla aleatoria
    	    for(int i=0; i<tama; i++){
    	    	Arreglo[i]=1+rand()%(11-1); // valores aleatorios del 1 al 10
    	    }
   ...
   for(int i=0; i< (tama/size);i++ ){
	...
	MPI_Reduce(&producto, // Valor local de PI
			   &productofinal,  // Dato sobre el que vamos a aplicar la operacion
			   1,	 // Numero de datos que vamos a reducir
			   MPI_DOUBLE,  // Tipo de dato que vamos a operar
			   MPI_PROD,  // Operacion que aplicaremos
	...
	if (rank == 0){
		...
		cout << "El valor de la multiplicacion de los datos aleatorios es: " <<productofinal<<endl;
	}
