...
int main(int argc, char *argv[])
{
    ...
    int xorinicial= 1, xorfinal; // Valor local de los elementos del arreglo
   ...
   for(int i=0; i< (tama/size);i++ ){
    	xorinicial = a[i] != xorinicial ;
    }


	MPI_Reduce(&xorinicial, // Valor local de PI
			   &xorfinal,  // Dato sobre el que vamos a aplicar la operacion
			   1,	 // Numero de datos que vamos a reducir
			   MPI_INT,  // Tipo de dato que vamos a operar
			   MPI_LXOR,  // Operacion que aplicaremos
	...
	if (rank == 0){
		...
		cout << "El resultado de aplicar xor a los valores aleatorios es: " <<xorfinal<<endl;
	}
