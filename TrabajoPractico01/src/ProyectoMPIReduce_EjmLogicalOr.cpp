...
int main(int argc, char *argv[])
{
    ...
    int orinicial= 1, orfinal; // Valor local de los elementos del arreglo
   ...
   for(int i=0; i< (tama/size);i++ ){
    	orinicial = a[i] || orinicial ;
    }


	MPI_Reduce(&orinicial, // Valor local de PI
			   &orfinal,  // Dato sobre el que vamos a aplicar la operacion
			   1,	 // Numero de datos que vamos a reducir
			   MPI_INT,  // Tipo de dato que vamos a operar
			   MPI_LOR,  // Operacion que aplicaremos
	...
	if (rank == 0){
		...
		cout << "El resultado de aplicar or a los valores aleatorios es: " <<orfinal<<endl;
	}
