...
int main(int argc, char *argv[])
{
   ...
   double min=99999999.999;
    ...
    cout<<"Valor minimo del proceso "<<rank<<": "<<min<<endl;
    local_min.val = min;
    local_min.rank = rank;


	MPI_Reduce(&local_min, // Valor local de PI
			   &global_min,  // Dato sobre el que vamos a aplicar la operacion
			   1,	 // Numero de datos que vamos a reducir
			   MPI_DOUBLE_INT,  // Tipo de dato que vamos a operar
			   MPI_MINLOC,  // Operacion que aplicaremos
			   0, // proceso que va a recibir el dato minimo
			   MPI_COMM_WORLD);
	...
	if (rank == 0){
		...
		cout << "El valor minimo de los datos aleatorios es: " << global_min.val<<endl;
		cout << "Se encuentra en el proceso: "<<global_min.rank<<endl;
	}
